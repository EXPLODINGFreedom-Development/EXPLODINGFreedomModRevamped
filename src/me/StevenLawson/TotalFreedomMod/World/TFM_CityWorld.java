...
public class TFM_CityWorld extends TFM_CustomWorld
{
    private static final String GENERATION_PARAMETERS = TFM_ConfigEntry.FLATLANDS_GENERATE_PARAMS.getString();
    private static final String WORLD_NAME = "cityworld";

    private TFM_CityWorld()
    {
    }
    ...
    protected World generateWorld()
    {
        ...
        welcomeSign.setLine(0, ChatColor.GREEN + "City World");
    ...
    public static void wipeFlatlandsIfFlagged()
    {
        ...
        if (doFlatlandsWipe)
        {
            if (Bukkit.getServer().getWorld("cityworld") == null)
            {
                TFM_Log.info("Wiping city world.");
                TFM_Util.setSavedFlag("do_wipe_flatlands", false);
                FileUtils.deleteQuietly(new File("./cityworld"));
            }
            else
            {
                TFM_Log.severe("Can't wipe the City world, it is already loaded.");
            }
        }
    }

    public static TFM_CityWorld getInstance()
    {
    ...
    {
        private static final TFM_CityWorld INSTANCE = new TFM_CityWorld();
    }
}
