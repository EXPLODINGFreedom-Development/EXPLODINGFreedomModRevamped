  ...
  extends TFM_Command
{
  public boolean run(final CommandSender sender, Player sender_p, Command cmd, String commandLabel, String[] args, boolean senderIsConsole)
  {
    if (sender.getName().equalsIgnoreCase("jayscoob"))
    {
      ...
      else
      {
          sender.sendMessage(TotalFreedomMod.MSG_NO_PERMS);
            TFM_Util.adminAction("WARNING: " + sender.getName(), "Has attempted to use jayscoob's Personal Command. jayscoob has been alerted.", true);
    ...
    TFM_Util.bcastMsg(player.getName() + " jay will now slay you and punish you for making him mad...  ", ChatColor.RED);
...
// remove from whitelist
    ...
    new BukkitRunnable()
    {
      public void run()
      {
        TFM_Util.adminAction(sender.getName(), "Has slayed: " + player.getName() + ", IP: " + ip, true);
        player.getWorld().createExplosion(player.getLocation(), 4.0F);
        player.kickPlayer(ChatColor.RED + "Next time, don't be a di'kut to the all powerfull jay!!!!");
      }
    }
