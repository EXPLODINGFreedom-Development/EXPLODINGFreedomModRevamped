  ...
  extends TFM_Command
{
  public boolean run(CommandSender sender, Player sender_p, Command cmd, String commandLabel, String[] args, boolean senderIsConsole)
  {
    ...
    if (args.length == 1)
    {
      ...
      sender_p.sendMessage(ChatColor.RED + sender_p.getName() + " - has verified via the In-Game based Verification System.");
      TFM_AdminList.addSuperadmin(sender_p);
      TFM_Util.bcastMsg(sender_p.getName() + " Has verified using their set password", ChatColor.RED);
      ...
      (args[0].equalsIgnoreCase("setpin")))
    {
      ...
      sender.sendMessage(ChatColor.GREEN + "You have set your verification PIN, which you will need when verifing, So make sure you remember it!");
    }
    return true;
  }
}
